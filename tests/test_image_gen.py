...
def test_router_has_five_backends_in_priority_order():
    ...
    assert (
        backends[-1].name == ImageGenBackend.STUb
        if hasattr(ImageGenBackend, "STUb")
        else backends[-1].name == ImageGenBackend.STUB
    )
    # Total of 5.
    assert len(backends) == 5
    types = {type(b).__name__ for b in backends}
    assert types == {
        "_LiteLLMImageBackend",
        "_ComfyUiFiboBackend",
        "_InvokeAiBackend",
...
def test_stub_returns_reproducible_output_for_same_record():
    ...
    with (
        patch(
            "gemini_hackathon.assets.image_gen._ComfyUiFiboBackend.is_available", return_value=False
        ),
        patch(
            "gemini_hackathon.assets.image_gen._InvokeAiBackend.is_available", return_value=False
        ),
        patch(
            "gemini_hackathon.assets.image_gen._UnslothStudioBackend.is_available",
            return_value=False,
        ),
        patch(
            "gemini_hackathon.assets.image_gen._LiteLLMImageBackend.is_available",
            return_value=False,
        ),
    ):
