...
def _step1_list(mo) -> None:
    ...
    if not raw_root.exists():
        mo.md(
            f"**No NCCE artefacts found at `{raw_root}`.** Run `mise run data:ncce:download` first."
        )
    ...
    for p in artefacts:
        kind = (
            ...
            if p.suffix == ".pdf"
                ...
                else ("placeholder" if p.suffix == ".json" else "?")
            )
        )
...
def _step4_query(mo) -> None:
    ...
    md_lines = [
        "| slug | kind | subject | year_level | sha256 (first 12) |",
        "|---|---|---|---|---|",
    ]
    for slug, kind, subject, year_level, sha in rows:
        md_lines.append(
            f"| `{slug}` | {kind} | {subject or ''} | {year_level or ''} | `{sha[:12]}…` |"
        )
    mo.md("\n".join(md_lines))
...
def _step5_plot(mo) -> None:
    ...
    fig = go.Figure(
        data=go.Heatmap(
            z=z, x=col_labels, y=row_labels, text=text, texttemplate="%{text}", colorscale="Greens"
        )
    )
    fig.update_layout(
    ...
    mo.md(
        f"**Rendered {len(rows)} rows × {len(cols)} columns.** (See the Plotly figure in the Gradio studio for the interactive view.)"
    )
