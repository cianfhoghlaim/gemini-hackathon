...
def get_firestore():
    ...
    Memoized per-process so multiple `get_firestore()` calls (e.g. one
    from the test setup + one from the pipeline's `_shared_fs()`) return
    the same InMemoryFirestore instance — otherwise the test's seeded
    artefact wouldn't be visible to the pipeline.
    """
