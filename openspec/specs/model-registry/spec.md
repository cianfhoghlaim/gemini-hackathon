...
The `gemini_hackathon.model_registry` module is the canonical single
source of truth for model strings routed through the gemini_hackathon
project. It exposes the `ModelRegistry` constructor (which guarantees
unique keys across all 8 families: `text_llm`, `ocr_vision`,
`image_gen`, `learning_graph`, `embedder`, `rerank`, `voice`,
`translation`) plus the canonical `PublicModelEntry` dataclass and the
`public_model_roster` / `public_tier_table` accessors that the docs,
the UI, and the submission materials must read from.

The historical duplicate registry at
`gemini_hackathon/models/__init__.py` was deleted in the
`2026-08-31-fix-critical-import-bugs-v1` change so there is exactly one
canonical model registry in the project.
## Requirements
