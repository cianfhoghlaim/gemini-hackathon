...
def test_subnation_table_has_eight_jurisdictions():
    ...
    assert slugs == {
        "ireland", "england", "northern_ireland", "scotland",
        "wales", "jersey", "guernsey", "isle_of_man",
    }
...
def test_build_learner_doc_shape():
    ...
    for required in ("learner_id", "subnation", "active_subject", "current_level",
                    "progress", "created_at", "journey_event_code"):
...
def test_write_learner_profile_offline_path():
    ...
    out = write_learner_profile(
        learner_id="test@school.ie", display_name="Test", subnation="scotland",
    )
...
def test_write_learner_profile_jersey_uses_jersey_palette():
    from gemini_hackathon.journey.level_0_pick_subnation.app import write_learner_profile
    out = write_learner_profile(
        learner_id="bob@school.je", display_name="Bob", subnation="jersey",
    )
