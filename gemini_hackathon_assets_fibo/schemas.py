"""gemini_hackathon_assets_fibo.schemas — database schemas for FIBO educational asset generation.

Lifted from `cianfhoghlaim/docs/sruth/tuath/fibo_generation/schemas.py`
(commit c853e360, "Phase 1 (atomic): tuatha/dlt/per_subject.py + 70 stub re-exports").

The Celtic-tourism-specific bits are NOT lifted. Everything education-relevant
is kept verbatim (the dataclass shapes are agnostic).

Used by:
  - gemini_hackathon/dlt_pipelines/ireland/leaving_certificate.py (W5)
  - gemini_hackathon/cocoindex_flows/ireland/leaving_certificate_embedding.py (W5)
  - gemini_hackathon/assets/fibo/assets.py (W10)
"""
...
class LearningOutcome:
    ...
    code: str  # e.g., "MA-LC-CH-1.2" (subject-stage-subject-LO-N.M)
    description: str
...
class SyllabusPage:
    """
    Schema for indexed syllabus pages.

    Each page from a curriculum PDF is converted to an image
    and embedded via ColPali (for visual document retrieval).
    """
...
class GeneratedAsset:
    ...
    style_medium: str  # "photograph", "digital_illustration", "diagram", etc.
    ...
    # Provenance (W2 + W14)
    ncca_policy_citations: list[str] = field(default_factory=list)  # ["SC-L1-L2, p.12", ...]
