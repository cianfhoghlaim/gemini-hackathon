...
SUBJECT_CATALOGUE: tuple[Subject, ...] = (
    ...
    Subject(
        "ncca_english_jc", "ncca.ie", Cycle.JC.value, "English", name_local="Béarla", short="Eng"
    ),
    Subject(
        "ncca_english_lc", "ncca.ie", Cycle.LC.value, "English", name_local="Béarla", short="Eng"
    ),
    ...
    Subject(
        "sqa_maths_higher", "sqa.org.uk", Cycle.HIGHER.value, "Mathematics Higher", short="Maths"
    ),
...
class Session:
    ...
    safeguarding_source_key: str | None = None
    palette_source_key: str | None = None
...
SUBNATIONS: tuple[SubnationMeta, ...] = (
    ...
    SubnationMeta(
        ...
        expansion=True,
        language_primary="English",
        language_secondary="Manx Gaelic",
    ),
)
...
AVAILABLE_SUBNATIONS: tuple[SubnationMeta, ...] = tuple(
    s for s in SUBNATIONS if s.available and not s.default
)
...
def subjects_for(subnation: str, cycle: str | None = None) -> list[Subject]:
    """Return the subjects for a given subnation (and optionally cycle)."""
    out = [
        s for s in SUBJECT_CATALOGUE if s.source_key == PALETTES_PER_JURISDICTION.get(subnation, "")
    ]
