...
async def search_syllabus_node(node_input: Any) -> dict[str, Any]:
    ...
    (node_input or {}).get("subnation", "ireland")
    subject = (node_input or {}).get("subject", "mathematics")

    # ── STUB: deterministic top-3 from a built-in sample (FIBO-format preview)
    stub_outcomes = [
        {
            "chunk_id": f"{subject}-001",
            "text_preview": f"Sample LO for {question[:60]}",
            "score": 0.92,
        },
        {
            "chunk_id": f"{subject}-002",
            "text_preview": f"Cross-curricular link for {question[:60]}",
            "score": 0.81,
        },
        {
            "chunk_id": f"{subject}-003",
            "text_preview": f"Assessment objective: apply the above to {question[:60]}",
            "score": 0.75,
        },
    ]
...
async def run_level_5(
    *,
    ...
    n1 = await search_syllabus_node(
        {"user_question": user_question, "subnation": subnation, "subject": subject}
    )
