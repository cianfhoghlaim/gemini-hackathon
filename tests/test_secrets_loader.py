...
def reload_secrets_loader():
    ...
    import gemini_hackathon.secrets_loader as mod  # noqa: PLC0415
...
def test_load_yaml_returns_project_and_entries(reload_secrets_loader):
    ...
    assert "entries" in config or any(
        isinstance(v, dict) and "env_var" in v
...
def test_load_from_gsm_resolves_required_secrets(monkeypatch, reload_secrets_loader):
    ...
    sample_config = {
        ...
        "entries": {
            "required-key": {"gsm_secret_id": "required-key", "env_var": "REQUIRED_KEY", "required": True},
            "optional-key": {"gsm_secret_id": "optional-key", "env_var": "OPTIONAL_KEY", "required": False},
        },
    }
...
def test_load_from_gsm_optional_missing_logs_warning(monkeypatch, reload_secrets_loader, caplog):
    ...
    assert any("GSM secret" in rec.message or "opt-key" in rec.message
